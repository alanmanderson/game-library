...
class GameManager:
    ...
    async def create_table(self, db: AsyncSession, player_id: str, preferred_color: Optional[str] = None, match_points: int = 5, time_control: str = "unlimited") -> Table:
        ...
        if preferred_color == "black":
            table = Table(id=table_id, black_player_id=player_id, status="waiting", match_points=match_points, time_control=time_control)
        else:
            table = Table(id=table_id, white_player_id=player_id, status="waiting", match_points=match_points, time_control=time_control)
